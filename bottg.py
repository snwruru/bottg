...
async def schedule_followup(bot: Bot, user_id: int, state: FSMContext, kp_text: str,
    ...
    try:
        ...
        await bot.send_message(
            chat_id=user_id,
            text="Мы не стараемся просто назвать минимальную цену «для заманухи». Наша задача — сразу показать более реалистичный расчёт. Но по многим объектам можно подобрать более выгодное решение. Позвоните, и я предложу оптимальный вариант под ваш бюджет. 8-903-244-16-66 Дмитрий"
        )
